# DiffusionDesk Compose Desktop Launch Script
param (
    [string]$JavaHome = "",
    [switch]$DryRun
)
...
if (-not $JavaHome) {
    ...
    Write-Host "Set JAVA_HOME or pass a JBR/JDK path with: .\scripts\run-compose.ps1 -JavaHome `"C:\Path\To\jbr`"" -ForegroundColor Yellow
    exit 1
}
...
Write-Host "Starting DiffusionDesk Compose Desktop..." -ForegroundColor Cyan
