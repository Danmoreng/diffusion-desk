...
diffusion_desk::json ToolService::execute_tool(const std::string& name, const diffusion_desk::json& arguments) {
    ...
    diffusion_desk::json err;
    err["error"] = "unknown_tool";
    return err;
}

diffusion_desk::json ToolService::get_library_items(const std::string& category) {
    if (!m_db) return diffusion_desk::json::array();
    return m_db->get_library_items(category);
}

diffusion_desk::json ToolService::search_history(const std::string& query) {
    if (!m_db) return diffusion_desk::json::array();
    return m_db->search_generations(query, 10);
}

diffusion_desk::json ToolService::get_vram_status() {
    float total = get_total_vram_gb();
    float free = get_free_vram_gb();
    diffusion_desk::json res;
    res["total_gb"] = total;
    res["free_gb"] = free;
