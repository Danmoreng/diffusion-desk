...
static std::shared_ptr<diffusion_desk::Database> g_db;
static std::shared_ptr<diffusion_desk::ResourceManager> g_res_mgr;
static std::shared_ptr<diffusion_desk::ToolService> g_tool_svc;
static std::shared_ptr<diffusion_desk::ServiceController> g_controller;
static std::shared_ptr<diffusion_desk::WsManager> g_ws_mgr;
static std::unique_ptr<diffusion_desk::TaggingService> g_tagging_svc;
static std::unique_ptr<diffusion_desk::HealthService> g_health_svc;
static std::unique_ptr<diffusion_desk::ImportService> g_import_svc;
static std::shared_ptr<diffusion_desk::JobService> g_job_svc;
static std::unique_ptr<diffusion_desk::ThumbnailService> g_thumb_svc;
...
int run_orchestrator(int argc, const char** argv, SDSvrParams& svr_params) {
    ...
    try {
        ...
        DD_LOG_ERROR("Failed to initialize core services: %s", e.what());
        return 1;
    }
    std::string sd_exe_path, llm_exe_path;
    fs::path bin_dir = fs::path(argv[0]).parent_path();
#ifdef _WIN32
    sd_exe_path = (bin_dir / "diffusion_desk_sd_worker.exe").string();
    llm_exe_path = (bin_dir / "diffusion_desk_llm_worker.exe").string();
#else
    sd_exe_path = (bin_dir / "diffusion_desk_sd_worker").string();
    llm_exe_path = (bin_dir / "diffusion_desk_llm_worker").string();
#endif
    std::vector<std::string> common_args;
    std::string passed_sd_model_arg;
    std::string passed_llm_model_arg;
    ...
    for (int i = 1; i < argc; i++) {
        ...
        // Filter out specific model args from common, capture them for specific distribution
        if (arg == "--diffusion-model") {
            if (i + 1 < argc) passed_sd_model_arg = argv[++i];
            continue;
        }
        if (arg == "--llm-model" || arg == "-lm") {
            if (i + 1 < argc) passed_llm_model_arg = argv[++i];
            continue;
        }
    ...
    g_thumb_svc = std::make_unique<diffusion_desk::ThumbnailService>(g_job_svc, g_db);
    g_job_svc->start();
    g_tagging_svc = std::make_unique<diffusion_desk::TaggingService>(g_db, llm_port, svr_params.listen_port, g_internal_token, svr_params.tagger_system_prompt);
    ...
    if (!initial_sd_model.empty()) DD_LOG_INFO("Initial SD model from args: %s", initial_sd_model.c_str());
    ...
    if (!preload_llm_model.empty()) {
        std::thread([preload_llm_model]() {
            if (wait_for_health_simple(llm_port, g_internal_token)) {
                diffusion_desk::json body; body["model_id"] = preload_llm_model;
                ...
                if (res && res->status == 200) DD_LOG_INFO("Successfully pre-loaded LLM model.");
                else DD_LOG_ERROR("Failed to pre-load LLM model.");
            }
        }).detach();
    }
    ...
    std::thread([&, svr_params]() {
        while (!is_shutting_down) {
            ...
            try {
                ...
                diffusion_desk::json status_msg; 
                    ...
                    [&](const char *data, size_t data_length) {
                        ...
                        while (true) {
                            ...
                            if (block.find("data: ") != std::string::npos) {
                                try {
                                    auto j = diffusion_desk::json::parse(block.substr(block.find("data: ") + 6));
                                    diffusion_desk::json msg; msg["type"] = "progress"; msg["data"] = j;
                                    g_ws_mgr->broadcast(msg);
                                } catch (...) {}
                            }
                        }
