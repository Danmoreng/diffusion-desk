...
The primary issue was a **Memory Layout Mismatch** between the DiffusionDesk server and the `stable-diffusion.cpp` library.
...
## 2. Solutions Applied
- **Struct Alignment:** Updated `DiffusionDesk/src/utils/common.hpp` to perfectly match the library's `sd_ctx_params_t` definition, including the missing circularity fields.
...
## 4. Maintenance Notes
If the `stable-diffusion.cpp` submodule is updated in the future, **always** verify that the `sd_ctx_params_t` struct in `stable-diffusion.h` matches the initialization in `DiffusionDesk/src/utils/common.hpp`.
